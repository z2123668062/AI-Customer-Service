...
async def create_session(user_id: int, title: str = "新对话") -> dict:
...
async def list_sessions(user_id: int) -> list[dict]:
    raw = await redis_client.hgetall(_session_meta_key(user_id))
    sessions = []
...
async def update_session_title(user_id: int, session_id: str, title: str) -> bool:
    raw = await redis_client.hget(_session_meta_key(user_id), session_id)
    if not raw:
        return False
    data = json.loads(raw)
    data["title"] = title
    await redis_client.hset(_session_meta_key(user_id), session_id, json.dumps(data))
    return True


async def delete_session(user_id: int, session_id: str) -> bool:
    await redis_client.delete(_session_key(user_id, session_id))
    await redis_client.hdel(_session_meta_key(user_id), session_id)
    return True